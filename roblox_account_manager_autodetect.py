...
# Automatically detect the Roblox Player path
def find_roblox_player():
    appdata_path = os.getenv("LOCALAPPDATA")
    roblox_versions_path = os.path.join(appdata_path, "Roblox", "Versions")
    
    # Find the most recent version of Roblox in the Versions folder
    if os.path.exists(roblox_versions_path):
        versions = sorted(os.listdir(roblox_versions_path), reverse=True)
        for version in versions:
            roblox_path = os.path.join(roblox_versions_path, version, "RobloxPlayerBeta.exe")
            if os.path.isfile(roblox_path):
                return roblox_path
    return None
...
def launch_client(username, temp_folder, roblox_player_path):
...
def launch_multiple_clients():
    ...
    roblox_player_path = find_roblox_player()
    if not roblox_player_path:
        print("Roblox Player executable not found.")
        return
    ...
    for i, username in enumerate(accounts):
        ...
        launch_client(username, temp_folder, roblox_player_path)
        time.sleep(1)
